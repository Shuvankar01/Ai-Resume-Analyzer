...
import { Users, TrendingUp, AlertCircle, LayoutDashboard, BrainCircuit, RefreshCw, Search, Sparkles } from 'lucide-react';
...
export default function RecruiterDashboard({ setUser }) {
  ...
  const tableColumns = useMemo(() => [
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <span className={`font-bold w-10 text-right ${
            row.score >= 80 ? 'text-[#00f3ff]' : row.score >= 50 ? 'text-yellow-400' : 'text-red-400'
          }`}>
            {row.score}%
          </span>
        </div>
    ...
    { 
      header: 'Status', 
      render: (row) => (
        <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-[0.15em] border ${
          row.score >= 80 ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-gray-500/10 text-gray-400 border-white/10'
        }`}>
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen bg-[#0a0a0f]">
        ...
        <div className="flex-1 ml-20 md:ml-64 p-10 space-y-10 animate-pulse">
